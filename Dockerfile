...
RUN cd /opt/mautic && composer require symfony/google-mailer


# Copy the patch file
# Copy the shell script into the container
COPY ./conf/google-mailer-patcher.sh /tmp/google_mailer_patcher.sh

# Make the script executable
RUN dos2unix /tmp/google_mailer_patcher.sh && \
    chmod +x /tmp/google_mailer_patcher.sh

# Run the script to patch the file
RUN /tmp/google_mailer_patcher.sh 

# Remove cache
RUN rm -rf /opt/mautic/var/cache/*
...
# Adding a dummy argument to force cache invalidation
ARG CACHEBUST=1
